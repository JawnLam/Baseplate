...
[![Version](https://img.shields.io/badge/version-v1.4.0-blue.svg)](VERSION.md)
...
See [`VERSION.md`](VERSION.md). This is **v1.4.0** — the lifecycle release: three earned lifecycle document classes (evolution roadmap / deferred-scope register, maintenance & stewardship, data lifecycle) and two recursive routes (`07-EVOLUTION.md`: MILESTONE-STACK for acting on a ripened roadmap entry, gate-first; ROADMAP-UPDATE for governed register changes) — Baseplate now covers a product's adulthood, not just its birth, with packaged-folder self-containment unweakened. Base: v1.3.0 (schema frozen; passes its own consistency audit; substrate-agnosticism shown by a cross-family golden session; the flywheel demonstrated by two shipped worked examples — the invented `Example-Product-Linkrot` and the real, production-recovered `Example-Product-Datestamper`; the **close-out packaging protocol** `06-CLOSE-OUT.md` packages every shipped stack into a self-contained handoff folder before the engagement counts as done; and, new in 1.3.0, that protocol's `Records/` reference sweep is bounded so a dangling controlled-identifier pointer in the record zone can no longer pass — closing PF-13). `CHANGELOG.md` is the authoritative release history.
