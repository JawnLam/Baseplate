...
Three structural consequences: (a) documents at different layers change at different speeds — binding them into one file couples their change rates and guarantees staleness; (b) **Layer 4 verifies Layer 1, never Layer 3** — acceptance criteria written against implementation cannot detect design errors; (c) no single artifact can serve the product, engineering, QA, and operations audiences at once without failing all of them. This is why the "one document a stranger could build from" ideal has always really been a small **stack**, not a single spec — a PRD deliberately answers *what/why* and stays silent on *how*; the SRS (ISO/IEC/IEEE 29148) is the closest any one document comes to the zero-questions handoff, and even it needs the design and verification layers to be truly buildable.

## 2. The menu (document classes per layer)

- **L0:** product brief / one-pager (problem, audience, why-now; Amazon's "Working Backwards" PR/FAQ is the famous variant); BRD (business objectives; enterprise/consulting contexts); MRD (market need + competitive landscape — largely absorbed into the PRD in modern practice); PR/FAQ variant for market-facing products.
- **L1:** PRD (features, scope, explicit non-goals, success metrics — how-agnostic by design); functional spec (exhaustive observable behavior: states, inputs, outputs, errors, edge cases; the Joel Spolsky "painless functional spec" lineage); SRS (the standardized ISO/IEC/IEEE 29148 union of functional + non-functional requirements, each numbered and testable — the closest single document to a zero-questions handoff); Gherkin-form acceptance criteria as the bridge to L4.
- **L2:** architecture document (components, boundaries, data flow, deployment topology — C4 or Kruchten's 4+1 views); ADRs (one short record per significant decision: context, options, choice, consequences); threat model (trust boundaries, attack surface, mitigations; STRIDE is the common method) where the product has a meaningful attack surface.
- **L3:** technical design doc / RFC per component (data models, algorithms, failure modes, alternatives rejected; Google design docs, and the IETF→Rust→React RFC lineage); interface contracts (API specs, schemas — the machine-checkable layer; the most frozen documents in the stack); data dictionary + ERD (field-level semantics, units, provenance); UX design spec wherever a human-facing interface exists.
...
- **L5:** runbook (setup, credentials, cadence, failure modes and responses, monitoring); SLO/SLI definitions and error budgets where availability matters; deployment/rollback plan and migration procedures.
