...
**The lifecycle release** — Baseplate now covers a product's adulthood, not just its birth. Scope pre-registered before the build (`_meta/release-v1.4.0-scope.md`); frozen v1.0 schema untouched throughout (all additions are the permitted additive case: new classes on existing Types + engine prose).

- **Three new document classes** in the `03-SELECTION.md` registry, each earned by interview answers: **evolution roadmap / deferred-scope register** (PI-9 or any deliberate deferral; `BASEPLATE_Narrative_Document`) — milestone rows with stable IDs, why-deferred, evidence awaited, extension point, **re-entry gate checklist**, dated status history; **maintenance & stewardship** (PI-8/PI-9; `BASEPLATE_Operations_Document`) — dependency re-verification cadence, change classification, acceptance-regression policy, versioning discipline, re-gate triggers; **data lifecycle** (PI-6; `BASEPLATE_Operations_Document`) — retention, export, deletion, departure, backup sensitivity.
- **New engine chapter `07-EVOLUTION.md`** with two recursive routes. **MILESTONE-STACK:** one `ready` register entry becomes gated stack documents — manifest verify → re-entry gate (an unmet item stops the route; waivers explicit and recorded) → scoped interview → selection delta locked before drafting → generation → Gate 1 over touched docs + full sweeps, Gate 2 iff builder-observable behavior changed → repackage per the REVISE discipline → register advances. **ROADMAP-UPDATE:** governed register maintenance — rationale + date per change, register-only audit (stable IDs, legal statuses, gates present, non-goal consistency), manifest regen, no stranger test. Routing tell: roadmap row → evolution; deviations entry/operator rejection → REVISE-STACK.
- **Route surface:** five routes everywhere routes are named (`00-START-HERE.md`, `AI-BOOTSTRAP.md`, readiness-statement rule); evolution session shapes; two new never-do lines (no milestone without its re-entry gate; no register edit outside ROADMAP-UPDATE).
