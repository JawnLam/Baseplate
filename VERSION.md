...
This is Baseplate **v1.0.0-rc.1** — the release candidate. Designed inside Operating-Volume-Engineering and passed through its gauntlet (traceability matrix + golden-session gate); its worked example (`Example-Product-Linkrot`) converged to a stranger-test PASS, which fired the schema-freeze trigger. `CHANGELOG.md` is the authoritative release history; this file and README derive their version from it.

| Identifier | Value | Notes |
|---|---|---|
| **Software** | v1.0.0-rc.1 | Engine (canon + 5 protocol chapters + bootstrap), 8 structural Types + class registry, 5-zone content model incl. the Grows-Through-Use portfolio zone, consistency-audit + stranger-test gates, portfolio failure catalog (PF-1..PF-6), manual VALIDATION-CHECKLIST |
