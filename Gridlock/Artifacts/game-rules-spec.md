...
| GL-RS-31 | **Punt:** net distance is one uniform integer draw from ⚙ `punt_band = [30, 45]`. Opponent takes over at `100 − (LOS + draw)`, but never inside their own 20-yard... precisely: if `LOS + draw ≥ 100`, touchback — opponent takes over at ⚙ `touchback_spot = 25` (their own 25, i.e., position 25 for them as new offense). Otherwise the opponent's starting position is `100 − (LOS + draw)`. The possession ends. | GL-AT-27 |
